...
function Modal() {
  const cartItemsContext = React.useContext(CartItemsContext);
  const modalRefContext = React.useContext(ModalRefContext);

  if (!cartItemsContext || !modalRefContext) {
    return null;
  }

  const { cartItems, orderTotal, handleClearCart } = cartItemsContext;
  const { modalRef } = modalRefContext;
